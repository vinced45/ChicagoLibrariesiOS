...
class LocationManager: NSObject {
    ...
    func start(_ monitorType: MonitorType = .locationOneTime, result: LocationClosure? = nil) {
        ...
        if !requestAuth() {
            locationCallback(LocationResult.error(LibraryError.permissions()))
            return
        }
        #if os(iOS)
            switch monitorType {
    ...
    func search(location: CLLocation, result: @escaping LocationClosure) {
        queue.async {
            ...
            geocoder.reverseGeocodeLocation(location) { placemarks, error in
                if let err = error {
                    self.locationCallback(LocationResult.error(LibraryError.init(errorType: err)))
                } else {
...
extension LocationManager: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, rangingBeaconsDidFailFor region: CLBeaconRegion, withError error: Error) {
        locationCallback(LocationResult.error(LibraryError.default()))
    }
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        locationCallback(LocationResult.error(LibraryError.init(errorType: error)))
    }
}
